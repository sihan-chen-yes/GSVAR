...
class Dataset:
    ...
    def __init__(
        ...
        if self.parser.test_every == 1:
            image_names = sorted(_get_rel_paths(f"{self.parser.data_dir}/images_{self.parser.factor}"), key=lambda x: int(x.split(".")[0].split("_")[-1]))
            assert len(image_names) == len(self.parser.image_names)
            if split == "train":
                # self.indices = [ind for ind in indices if "_train_" in image_names[ind]]
                self.indices = [ind for ind in indices if ind % 10 == 0]
            else:
