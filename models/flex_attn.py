...
def _generate_var_mask_mod(offsets, use_ref=False):
    ...
    document_id = _offsets_to_doc_ids_tensor(offsets).repeat(2) if use_ref else _offsets_to_doc_ids_tensor(offsets)
...
def _generate_var_infer_mask_with_kv_cache(lengths, use_ref=False):
    kv_len = sum(lengths) * 2 if use_ref else sum(lengths)
    def var_mask_mod(b, h, q_idx, kv_idx):
...
class FlexAttn(nn.Module):
    def __init__(
            self, lengths:list, mask_type:str, use_ref:bool, B, H, L:int, auto_padding=False
    ):
        """
        ...
        if mask_type == "var":
            self.mask_mod = _generate_var_mask_mod(self.offsets, use_ref=use_ref)
        elif mask_type == "causal":
            self.mask_mod = _causal_mask
        elif mask_type == 'var_infer_mask_with_kv_cache':
            self.mask_mod = _generate_var_infer_mask_with_kv_cache(self.lengths, use_ref=use_ref)
        else:
    ...
    def extra_repr(self) -> str:
        tail = ''
        return f'lengths: {self.lengths} {tail}'
