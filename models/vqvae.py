...
class VQVAE(nn.Module):
    ...
    def fhat_to_img(self, f_hat: torch.Tensor, cond_features=None) -> torch.Tensor:
        return self.decoder(self.post_quant_conv(f_hat), cond_features=cond_features).clamp_(-1, 1)
    ...
    def img_to_idxBl(self, inp_img_no_grad: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.LongTensor]:    # return List[Bl]
        f = self.quant_conv(self.encoder(inp_img_no_grad, return_cond_features=False))
        return self.quantize.f_to_idxBl_or_fhat(f, to_fhat=False, v_patch_nums=v_patch_nums)

    def get_cond_features(self, inp_img_no_grad: torch.Tensor):
        _, cond_features = self.encoder(inp_img_no_grad, return_cond_features=True)
        return cond_features

    def idxBl_list_to_img(self, ms_idx_Bl: List[torch.Tensor], same_shape: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
    ...
    def idxBl_to_img(self, ms_idx_Bl: torch.Tensor, same_shape: bool, last_one=False, last_layer_residual_f=None, cond_features=None) -> Union[List[torch.Tensor], torch.Tensor]:
        B = ms_idx_Bl.shape[0]
        split_sizes = [n ** 2 for n in self.v_patch_nums]
        # Split tensor into list of (1, L_i) = （1, s_i * s_i)
        ms_idx_Bl = list(torch.split(ms_idx_Bl, split_sizes, dim=1))
        ms_h_BChw = []
        for idx_Bl in ms_idx_Bl:
            l = idx_Bl.shape[1]
            pn = round(l ** 0.5)
            ms_h_BChw.append(self.quantize.embedding(idx_Bl).transpose(1, 2).view(B, self.Cvae, pn, pn))
        if last_layer_residual_f is not None:
            ms_h_BChw[-1] += last_layer_residual_f
        return self.embed_to_img(ms_h_BChw=ms_h_BChw, all_to_max_scale=same_shape, last_one=last_one, cond_features=cond_features)

    def embed_to_img(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale: bool, last_one=False, cond_features=None) -> Union[List[torch.Tensor], torch.Tensor]:
        if last_one:
            return self.decoder(self.post_quant_conv(self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=True)), cond_features=cond_features).clamp_(-1, 1)
        else:
            return [self.decoder(self.post_quant_conv(f_hat), cond_features=cond_features).clamp_(-1, 1) for f_hat in self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=False)]
