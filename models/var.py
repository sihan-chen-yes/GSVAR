...
class VAR_RoPE(nn.Module):
    def __init__(
        ...
        lvl_1L = d.transpose(1, 2)[:, 0].contiguous()
        self.register_buffer("lvl_1L", lvl_1L)
        self.view_num = 2 if self.args.use_ref else 1
        d = repeat(d, 'b l c -> b (v l) c', v=self.view_num)
        dT = d.transpose(1, 2)  # dT: 11L
        attn_bias_for_masking = torch.where(d >= dT, 0.0, -torch.inf).reshape(
            1, 1, (self.L + context_token - 1) * self.view_num, (self.L + context_token - 1) * self.view_num
        ) # (v l) x (v l)
        ...
        # only used for training speed up
        lengths = [context_token] + [pn * pn for pn in self.patch_nums[1:]]
        mask_type = "var"
        try:
            self.attn_fn = FlexAttn(
                lengths,
                mask_type=mask_type,
                use_ref=self.args.use_ref,
                B=None,
                H=None,
                L=self.L + context_token - 1,
                auto_padding=False,
            )
        except Exception as e:
            print(f"[Warning] FlexAttn init failed: {e}, not using flex_attention...")
            self.attn_fn = None
    ...
    def autoregressive_infer_cfg(
            ...
            more_smooth=False, lr_inp_scale=None, tile_flag=False, normalize=False,
        ...
        # B 3 H W -> B C pn pn
        lr_inp_emb, _ = self.con_embedding(lr_inp)
        lr_inp_emb = lr_inp_emb.view(B, self.C, -1).permute(0, 2, 1) # B C pn*pn -> B pn*pn C
        assert lr_inp_emb is not None
        assert lr_inp_emb.shape[1] == self.context_token - 1

        # forcing ref, use gt ref to guide
        lr_idx_Bl, idx_N_list = self.vae_proxy[0].img_to_idxBl(lr_inp)

        sos = lr_inp_emb

        if self.label_B_flag:
            cond_BD = self.class_emb(label_B)
        sos = torch.cat((sos, cond_BD.unsqueeze(1)), dim=1) # B pn*pn+1 C

        lvl_pos = self.lvl_embed(self.lvl_1L) # 1 L C
        next_token_map = sos + self.pos_start + lvl_pos[:, :self.first_l] # B pn*pn+1 C
        ...
        if lr_inp_scale is not None:
            next_token_map[:, -1, :] = next_token_map[:, -1, :] + self.word_embed(lr_inp_scale[:, 0])
        for b in self.blocks:
            b.attn.kv_caching(True)
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ratio = si / self.num_stages_minus_1
            if si > 0:
                freqs_cis = self.freqs_cis.repeat(B, 1, 1)[:, cur_L:cur_L + pn * pn]
                cur_L += pn * pn
            else:
                freqs_cis = self.freqs_cis.repeat(B, 1, 1)[:, 0:self.context_token]
            ...
            for i, b in enumerate(self.blocks):
                ...
                x = b(x=x, cond_BD=cond_BD_or_gss, freqs_cis=freqs_cis, attn_bias=None, attn_fn=None)
        ...
        # change discrete features to continuous
        h_BChw_diff = self.idx_Bl_to_residual_f(idx_Bl, last_layer_cond, cfg=cfg)
        f_hat += h_BChw_diff

        if tile_flag:
            # only return f_hat to form bigger res f_hat
            return f_hat
        else:
            # VAE bridge
            cond_features = [self.vae_proxy[0].get_cond_features(lr_inp)] if self.args.use_bridge else None
            ret_img = self.vae_proxy[0].fhat_to_img(f_hat, cond_features=cond_features)
            if not normalize:
                ret_img.add_(1).mul_(0.5)  # de-normalize, from [-1, 1] to [0, 1]
            return ret_img

    def idx_Bl_to_residual_f(self, idx_Bl, last_layer_cond, cfg=1.5):
        """
        Args:
            idx_Bl: idx of last layer [B, L]
            last_layer_cond: last layer condition [B, L, Cvae] (after transformer)
            cfg:  t
        ...
        if cfg != 1.0:
            last_layer_cond = torch.cat([last_layer_cond, last_layer_cond], dim=0)
        ...
        h_BChw_diff = self.diffloss.sample(
            z=last_stage_cond, temperature=1.0, cfg=cfg
        )
    ...
    def forward(self, x_BLCv_wo_first_l: torch.Tensor, label_B, lr_inp, gt, text_hidden,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            lr_inp_emb, out_rgbs = self.con_embedding(lr_inp)
            sos = lr_inp_emb.view(B, self.C, -1).permute(0,2,1)
            if self.label_B_flag:
                cond_BD = self.class_emb(label_B)
        ...
        for i, b in enumerate(self.blocks):
            x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss,  
                    freqs_cis=freqs_cis, attn_bias=attn_bias, attn_fn=self.attn_fn)
        ...
        with torch.no_grad():
            ...
            idx_BL_last_sampled = idx_BL_sampled[:, self.L + self.context_token - 1 - self.last_level_pns : ]

            h_BChw_diff = self.idx_Bl_to_residual_f(idx_BL_last_sampled, last_layer_cond, cfg=1.0) # no cfg here
            idx_Bl = idx_BL_sampled[:, self.context_token - 1 :]
            pred_img = self.vae_proxy[0].idxBl_to_img(idx_Bl, same_shape=False, last_one=True, last_layer_residual_f=h_BChw_diff)

        last_stage_discrete_embed = self.vae_quant_proxy[0].embedding(idx_BL_last_sampled)
        ...
        if not self.args.use_ref_view_loss:
            last_layer_cond = rearrange(last_layer_cond, '(b v) l c -> b v l c', v=self.view_num)[:, 0]
            last_layer_gt_continuous = rearrange(last_layer_gt_continuous, '(b v) l c -> b v l c', v=self.view_num)[:, 0]
        ...
        ret = {
            ...
            "diff_loss": diff_loss,
            "pred_img": pred_img,
            "mask_wo_prev_stages": mask_wo_prev_stages
        }
        return ret   # logits BLV, V is vocab_size
