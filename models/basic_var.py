...
dropout_add_layer_norm = fused_mlp_func = memory_efficient_attention = flash_attn_func = flex_attention = None
try:
...
try: from torch.nn.attention.flex_attention import flex_attention  # qkv: BLHc, ret: BLHcq
except ImportError: pass
...
class SelfAttention_RoPE(nn.Module):
    def __init__(
        ...
        if lora_config is None:
            ...
            target_modules = lora_config.get('lora_target_modules', "qv")
            enable_lora = [True if m in target_modules else False for m in ['q', 'k', 'v']]
            self.mat_qkv = lora.MergedLinear(embed_dim, embed_dim * 3, bias=False, r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, enable_lora=enable_lora)
            self.proj = lora.Linear(embed_dim, embed_dim, r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout) if 'o' in target_modules else nn.Linear(embed_dim, embed_dim)
        self.q_bias, self.v_bias = nn.Parameter(torch.zeros(embed_dim)), nn.Parameter(torch.zeros(embed_dim))
        self.register_buffer('zero_k_bias', torch.zeros(embed_dim))
        ...
        self.attn_drop: float = attn_drop
        self.using_flash = flash_if_available and flash_attn_func is not None
        self.using_flex = flash_if_available and flex_attention is not None
        self.using_xform = flash_if_available and memory_efficient_attention is not None
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, freqs_cis, attn_bias, attn_fn=None):
        B, L, C = x.shape

        out = self.mat_qkv(x)
        qkv = (out + torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)).to(out.dtype)).view(B, L, 3, self.num_heads, self.head_dim)  # (B, L, 3, num_heads, head_dim)
        main_type = qkv.dtype
        # qkv: BL3Hc
        
        using_flash = self.using_flash and attn_bias is None and qkv.dtype != torch.float32
        using_flex = self.using_flex and attn_fn is not None and qkv.dtype != torch.float32
        ...
        dropout_p = self.attn_drop if self.training else 0.0
        if using_flash: # for inference
            oup = flash_attn_func(q.to(dtype=main_type), k.to(dtype=main_type), v.to(dtype=main_type), dropout_p=dropout_p, softmax_scale=self.scale).view(B, L, C)
        elif using_flex: # for training
            oup = attn_fn(q.to(dtype=main_type), k.to(dtype=main_type), v.to(dtype=main_type), scale=self.scale).transpose(1, 2).reshape(B, L, C)
        elif self.using_xform:
            oup = memory_efficient_attention(q.to(dtype=main_type), k.to(dtype=main_type), v.to(dtype=main_type), attn_bias=None if attn_bias is None else attn_bias.to(dtype=main_type).expand(B, self.num_heads, -1, -1), p=dropout_p, scale=self.scale).view(B, L, C)
        else:
            oup = slow_attn(query=q, key=k, value=v, scale=self.scale, attn_mask=attn_bias, dropout_p=dropout_p).transpose(1, 2).reshape(B, L, C)
    ...
    def extra_repr(self) -> str:
        return f'using_flash={self.using_flash}, using_flex:{self.using_flex}, using_xform={self.using_xform}, attn_l2_norm={self.attn_l2_norm}'
...
class AdaLNSelfAttn_RoPE(nn.Module):
    def __init__(
        ...
        self.attn = SelfAttention_RoPE(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop,
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward_part(self, x, cond_BD, freqs_cis, attn_bias, attn_fn=None):   # C: embed_dim, D: cond_dim
        BV, L, D = x.shape
        V = 2 if self.args.use_ref else 1
        B = BV // V
        if self.args.use_ref and self.block_idx in self.args.fusion_block_idx:
            #do token-wise self-attention along render and ref view
            x = rearrange(x, '(b v) l d -> b (v l) d', v=V)
            cond_BD = rearrange(cond_BD, '(b v) d -> b v d', v=V)
            freqs_cis = rearrange(freqs_cis, '(b v) l d -> b (v l) d', v=V)
            if self.shared_aln:
                gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(
                    2)  # 116C + B16C =unbind(2)=> 6 B1C
            else:
                if self.ada_lin_flag:
                    ada_lin = self.ada_lin(cond_BD).view(B, V, 6, self.C)
                    ada_lin = repeat(ada_lin, 'b v n d -> b (v l) n d', l=L)
                    gamma1, gamma2, scale1, scale2, shift1, shift2 = ada_lin.unbind(2)
        else:
            attn_bias = attn_bias[:, :, :L, :L] if attn_bias is not None else None
            if self.shared_aln:
                gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(
                    2)  # 116C + B16C =unbind(2)=> 6 B1C
            else:
        ...
        if self.ada_lin_flag:
            x = x + self.drop_path(self.attn( self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), freqs_cis=freqs_cis, attn_bias=attn_bias, attn_fn=attn_fn).mul_(gamma1))
            x = x + self.drop_path(self.ffn( self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed when FusedMLP is used
        else:
            x = x + self.drop_path(self.attn( self.ln_wo_grad(x), freqs_cis=freqs_cis, attn_bias=attn_bias, attn_fn=attn_fn))
    ...
    def forward(self, x, cond_BD, freqs_cis, attn_bias, attn_fn=None):
        if self.use_checkpoint:
            # avoid ckpt bugs when using LoRA
            # x.requires_grad_(True)
            x = checkpoint.checkpoint(self.forward_part, x, cond_BD, freqs_cis, attn_bias, attn_fn, use_reentrant=False)
        else:
            x = self.forward_part(x, cond_BD, freqs_cis, attn_bias, attn_fn=attn_fn)
...
class AdaLNBeforeHead(nn.Module):
    ...
    def forward(self, x_BLC: torch.Tensor, cond_BD: torch.Tensor):
        if self.use_checkpoint:
            x = checkpoint.checkpoint(self.forward_part, x_BLC, cond_BD, use_reentrant=False)
        else:
