...
def main(args):
    ...
    dataset_val = PairedDataset(dataset_path=args.dataset_path, split="val", tokenizer=net_difix.tokenizer)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # Reconstruction loss
                loss_l2 = F.mse_loss(x_tgt_pred[:, 0].float(), x_tgt[:, 0].float(), reduction="mean") * args.lambda_l2
                loss_lpips = net_lpips(x_tgt_pred[:, 0].float(), x_tgt[:, 0].float()).mean() * args.lambda_lpips
                loss = loss_l2 + loss_lpips
                
                # Gram matrix loss
                if args.lambda_gram > 0:
                    if global_step > args.gram_loss_warmup_steps:
                        x_tgt_pred_renorm = t_vgg_renorm(x_tgt_pred[:, 0] * 0.5 + 0.5)
                        ...
                        x_tgt_renorm = t_vgg_renorm(x_tgt[:, 0] * 0.5 + 0.5)
