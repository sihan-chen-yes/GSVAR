...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        num_classes, dataset_train, dataset_val = build_gs_dataset(
            args.json_path, final_reso=args.data_load_reso, tokenizer=None, null_text_ratio=0.3, original_image_ratio=0.0, args=args,
        )
        types = str((type(dataset_train).__name__, type(dataset_val).__name__))

        ld_val = DataLoader(
            dataset_val, num_workers=args.workers, pin_memory=True,
            batch_size=1, sampler=EvalDistributedSampler(dataset_val, num_replicas=dist.get_world_size(), rank=dist.get_rank()),
    ...
    if args.use_lora:
        # use lora
        loralib.mark_only_lora_as_trainable(var_wo_ddp)
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        [me.add_meter(x, misc.SmoothedValue(fmt='{median:.2f} ({global_avg:.2f})')) for x in ['l2_loss', 'lpips_loss', 'gram_loss']]
        ...
        for it, (batch) in me.log_every(start_it, iters_train, ld_train, 300 if iters_train > 8000 else 5, header):
            ...
            if (it + 1) % args.eval_freq == 0 or (it + 1) % iters_train == 0:
                ...
                if dist.is_local_master():
                    local_out_cur_ckpt = os.path.join(args.local_out_dir_path, f'ar-ckpt-ep_{ep+1:04d}.pth')
